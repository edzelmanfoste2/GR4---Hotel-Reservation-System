...
//The Cancellation form informations needed
        ...
        reservationLabel.setText("Reserve Room Number");//Reservation number needed. Not the room number.
...
//The panel body for the header
    ...
    public void actionPerformed(ActionEvent e) {
        if (e.getSource() == submitButton) {
            ...
            try {
                dispose();
                Connection cancelForm = DriverManager.getConnection("jdbc:mysql://localhost:3306/hotelmanagementsystem", "root", "adminsql");
                System.out.println("Connected to the database");

                PreparedStatement preparedStatement = cancelForm.prepareStatement("insert into tbl_cancellation values(?, ?, ?, ?)");
                ...
                String updateDB = "update tbl_roomsavailabilityvc set Availability = 0 where Rooms_Availability = ?";
                try (PreparedStatement updateAvailability = connect.prepareStatement(updateDB)){
                //PreparedStatement updateAvailability = connect.prepareStatement("update tbl_checkinform set Availability = 1 where RoomNumber = ?");
                updateAvailability.setString(1, reservationText.getText());
                 int rowsAffected = updateAvailability.executeUpdate();
                    if (rowsAffected > 0) {
                        System.out.println("Room availability updated successfully");
                    } else {
                        System.out.println("Room availability update failed");
                    } 
                } 
